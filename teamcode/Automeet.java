...

{
    ...
    telemetry.update();
    rbga.outtake_cam.setPosition(rbga.out_cam[0]);
  //  rbga.intake_handle(0);//
    rbga.intake_grab(2);// lock
...
//        }
    ...
    while (!rbga.init(true) && !isStopRequested())
 ...
 public int huskydet() {
    ...
    int ID_count = 0;
     ...
     rateLimit.expire();
     rbga.huskyLens.selectAlgorithm(HuskyLens.Algorithm.OBJECT_CLASSIFICATION);

     if (!rbga.huskyLens.knock()) {
//         telemetry.addLine("Restart the program  !!!");
//         telemetry.addData(">>", "Problem communicating with " + rbga.huskyLens.getDeviceName());
//         telemetry.update();
     } else {
     ...
     while (!isStopRequested()&&!isStarted()) {
        // if(out>0) out=0;
         ...
         rateLimit.reset();
         HuskyLens.Block[] blocks = rbga.huskyLens.blocks();

         for (HuskyLens.Block block_item : blocks) {
...
//
 ...
 void autoroute() {
     ...
     {
        rbga.bgrab();
       rbga.bstrafetowall(false);
       rbga.bstraight(false);
      rbga.bstrafefromwall(false);
      if(!rbga.baseblue) rbga.aouttake2l();
      else rbga.aouttake2r();
       rbga.bstrafetowall(true);
         for (int j = 0; j < 2; j++) {
             if(rbga.timeend) break;
             rbga.bstraight(true);
             rbga.bstrafefromwall(true);
             rbga.bgrab();
             rbga.bstrafetowall(false);
             rbga.bstraight(false);
             sleep(rbga.outtake_2_delay);
             rbga.bstrafefromwall(false);
             if (!rbga.baseblue) rbga.aouttake2l();
             else rbga.aouttake2r();
             rbga.bstrafetowall(true);
         ...
         for (int i = 0; i < 2; i++) {
          ...
          //   sleep(300000);
             rbga.bgrab();
             rbga.astraight(false);
             rbga.ain();
           //  sleep(30000);
           if(!rbga.baseblue)  rbga.aouttake2r(); else rbga.aouttake2l();
           rbga.aout();
         }
     }
     rbga.stop_drive();
